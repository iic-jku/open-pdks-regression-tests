...
Nothing found for the four below.
...
## 5. New issue: sidewall coefficient halved

**Title**

```
Sidewall capacitance is emitted at half the tech-file coefficient
```
...
The ihp-sg13g2 extraction deck writes

```
defaultsidewall allm1 metal1 28.735 -0.057
```

Two parallel Metal1 wires, 0.5 um wide, 50 um long, separation swept. What Magic emits is `14.3675 * L / (s - 0.05)`, to five digits at every spacing over a 16-fold range: half the coefficient, with the offset quantised to whole lambda.

| separation [um] | k*L/(s+off) from the deck | Magic |
| --- | --- | --- |
| 0.2 | 10.0472 | 4.7892 |
| 0.4 | 4.1888 | 2.0525 |
| 0.8 | 1.9337 | 0.9578 |
...
`ExtTech.c` halves the parsed value with the comment that both edges of a pair get visited and each should contribute half. On this geometry the second visit does not happen. Writing the same coefficient through the explicit `sidewall` keyword instead of `defaultsidewall` gives an identical result, so it is not specific to the default form.

Two independent checks that the full value is the intended one:

- KLayout's kpex reads the identical coefficients for this PDK (`sidewall 28.735, offset -0.057` in its own tech file) and its 2.5D engine emits the full value at every spacing, matching the deck formula to six digits. The two tools agree to six digits on `areacap`, `perimc` and `overlap`, and differ by exactly two only here.
- A mesh-converged FasterCap solve of the same structures (raw-matrix asymmetry 0.03 % to 0.16 %) gives 6.2195, 3.6468, 2.0777, 1.0585 and 0.4364 fF for the five spacings. Magic is below that at every single spacing, by 23 % to 56 %. The unhalved deck formula straddles it, high at close spacing and low at wide, which is what a fitted `1/(s+off)` form looks like against a curve of a different shape.

A fitted model does not lie below its own fitting data at every point. A fitted model divided by two does.
